...
- 决策：
  ...
  - **双产物**（buildApp 的 rolldown input 两入口）：`dist/index.js`（自托管：导出 app + `import.meta.main` 条件启动——直接运行启动、被平台 import 不启动）与 `dist/app.js`（无状态：只导出 app）。模块表随产物携带（阻塞③解决）。
...
```
Vite 插件 src/plugin.ts（vite.config.ts 注册）
...
- **无 server/index.ts**：服务器入口由插件生成（虚拟模块 kiiii:server，内容 = createKiiiiServer
