...
- 扩展预留：default 导出对象时路由加方法段（`/kiiii/:name/:method`），MVP 不做。
...
Vite 插件 src/index.ts（vite.config.ts 注册）
  ├─ 配置：kiiii({ pattern?, prefix?, timeout? })——pattern 是唯一事实来源
  │     必填（如 /src/**\/*.server.ts，支持数组），目录与后缀完全放开；无服务器入口文件
  ├─ config 钩子：build 时声明 SSR 构建（ssr: true + rolldownOptions.input =
  │     虚拟入口 kiiii:start）——Vite 标准流程注入完整服务器环境；dev 不受影响
  ├─ buildApp 钩子：一个 vp build 完成两个环境——先服务器（→ 打包根 dist），
  │     再重定向客户端 outDir → {打包根}/clients 并构建（清掉继承的服务器入口 input）
  ├─ resolveId(source, importer, options)
  │     kiiii:start / kiiii:modules → 虚拟 id；客户端 import 命中 pattern
  │     （createFilter 精确匹配）&& !options.ssr → 虚拟 stub id
  ├─ load(id)
  │     入口虚拟模块 → createKiiiiServer 调用代码；模块表虚拟模块 → import.meta.glob
  │     （vite:import-glob 转换，注意不能带泛型——虚拟模块按 JS 解析）；stub → fetch 调用
  └─ configureServer(server)
        dev 接线：invalidate 模块表虚拟模块 + ssrLoadModule（新增文件即用，无目录扫描）

runtime src/server.ts（dev/prod 共用）
  ├─ createKiiiiHandler：远程调用分发 handler（h3）
  ├─ createKiiiiServer：生产服务器封装（远程调用 + 静态资源 + SPA fallback + listhen）
  └─ KiiiiError / KiiiiContext / isKiiiiError
```
...
- **无 server/index.ts**：服务器入口由插件生成（虚拟模块 kiiii:start，内容 = createKiiiiServer
...
- **createKiiiiServer 封装**（同一 h3 app、单端口组合四件事）：
...
- 客户端不引入 KiiiiError 类，reject 的 Error 挂属性即可：`catch (e) { if (e.code === "EMPTY_NAME") }`
