...
- 已知待办（CF 第二版）：app 产物 chunk 链仍含 `import("node:path")`（startServer 与 createKiiiiApp 同 chunk，tree-shaking 不隔离）——wrangler 打包可能报错；解法：startServer 拆独立文件（server-node.ts）使平台产物链彻底无 node 引用，再实测 CF。
